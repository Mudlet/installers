...
if [ -z "${ptb}" ]; then
  perl -pi -e "s|icons/.*\\.icns|icons/mudlet_ptb.icns|i" "$BUILD_DIR/installers/appdmg/mudlet-appdmg.json"
else
  if [ -z "${release}" ]; then
    perl -pi -e "s|icons/.*\\.icns|icons/mudlet_dev.icns|i" "$BUILD_DIR/installers/appdmg/mudlet-appdmg.json"
  else
    perl -pi -e "s|icons/.*\\.icns|icons/mudlet.icns|i" "$BUILD_DIR/installers/appdmg/mudlet-appdmg.json"
  fi
fi

# Last: build *.dmg file
appdmg "$BUILD_DIR/installers/appdmg/mudlet-appdmg.json" "${HOME}/Desktop/$(basename "${app%.*}").dmg"
